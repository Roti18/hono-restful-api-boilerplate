...
{#if $selectedDetail}
	<div
		...
		class="mx-auto max-w-270 py-16 pb-24 max-md:py-8 max-md:pb-16"
	>
		<!-- Header -->
		<div class="mb-20 max-w-190 max-md:mb-12">
			...
			>
				{#each ['Archive', ...$navigationPath, $selectedDetail.id] as item, i}
					<span class="text-gray-300">{item}</span>
					{#if i < ['Archive', ...$navigationPath, $selectedDetail.id].length - 1}
