...
const target = args[1]
const extra = args[2] // id for update/delete, or first param
const restParams = args.slice(2) // for insert/update pairs
...
const printTable = (rows: any[]) => {
  ...
  // Header
  console.log(headers.map((h, i) => h.toUpperCase().padEnd(colWidths[i])).join('  '))
...
const run = async () => {
  try {
    switch (command) {
      ...
      case 'list-rows': {
        if (!target) throw new Error('Table name required')
        const res = await db.execute(`SELECT * FROM ${target} ORDER BY created_at DESC LIMIT 50`)
      ...
      case 'insert': {
        ...
        if (Object.keys(data).length === 0) throw new Error('No data provided (key=value)')
